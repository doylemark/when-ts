...
export function exactMonthDate(strategy: Strategy = "override"): Rule {
  ...
  // "third of march", "march third", "march 3rd", "3rd march", "march 3"
  // "1st of september", "sept. 1st", "march 7th", "october 21st"
  // "twentieth of december", "march 10th", "jan. 6"
  // "february", "october", "jul.", "june"

  // Groups:
  // 1. ordinal day before month (like "third of" or "3rd")
  ...
  return createRule(
    ...
    (match, context, options, ref) => {
      ...
      // Check ordinal before month (strip " of" suffix if present)
      if (ord1 !== "") {
        const cleanOrd = ord1.replace(/\s+of$/, "");
